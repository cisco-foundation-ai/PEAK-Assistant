...
async def run_tests():
    ...
    if auth_mgr.ensure_configured():
        ...
        print("❌ Authentication is not configured. Please check your environment variables.")

    auth_params = await auth_mgr.get_auth_params()
    if auth_params:
        print(f"✅ Successfully retrieved authentication parameters.\n{auth_params}\n")
    else:
        print("❌ Failed to retrieve authentication parameters. Please check your configuration.")
    ...
    if llm_client:
        ...
        print("❌ Failed to create Azure OpenAI client. Please check your configuration.")
