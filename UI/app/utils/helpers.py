...
def extract_report_md(messages):
    ...
    if hasattr(messages, "messages"):
        ...
        report_md = next(
            (
                ...
                if getattr(m, "source", None) == "summarizer_agent"
            ),
            None,
        )
        ...
        if not report_md:
            ...
            logger.info(
                "extract_report_md: No summarizer_agent found, trying fallbacks..."
            )
            ...
            if not report_md:
                ...
                for m in reversed(messages.messages):
                    ...
                    if (
                        ...
                        logger.info(
                            f"extract_report_md: Found report-like content from source: {getattr(m, 'source', 'unknown')}"
                        )
                        report_md = content
                        break
            ...
            if not report_md:
                ...
                longest_msg = max(
                    messages.messages,
                    key=lambda m: len(getattr(m, "content", "")),
                    default=None,
                )
                if longest_msg and hasattr(longest_msg, "content"):
                    logger.info(
                        f"extract_report_md: Using longest message from source: {getattr(longest_msg, 'source', 'unknown')}"
                    )
    ...
    if not report_md:
        if isinstance(messages, str):
            ...
            logger.warning(
                "extract_report_md: No suitable content found, falling back to string representation"
            )
            # Instead of str(messages), try to extract any readable content
            if hasattr(messages, "__dict__"):
                logger.info(
                    f"extract_report_md: Message attributes: {list(messages.__dict__.keys())}"
                )
            report_md = "Error: Could not extract report content from agent response. Please try again."

    logger.info(
        f"extract_report_md: Extracted report length: {len(report_md) if report_md else 0}"
    )
    return report_md
