...
from planning_assistant.__main__ import plan_hunt as async_plan_hunt
...
def embeddable_object(
    ...
    resource = types.TextResourceContents(
        uri=AnyUrl(uri), text=data, mimeType=content_type
    )
...
async def hypothesizer(
    research_document: str, local_context: Optional[str] = None
) -> types.EmbeddedResource:
    """
    ...
    result = await async_hypothesizer("", research_document, local_context or "")
...
async def hypothesis_refiner(
    ...
    result = await async_refiner(
        ...
        research_document=research_document or "",
...
async def data_discovery(
    ...
    mcp_command = mcp_command or os.getenv("SPLUNK_MCP_COMMAND", "")
    mcp_args = mcp_args or os.getenv("SPLUNK_MCP_ARGS", "")
...
def main() -> None:
    ...
    print("Starting the PEAK Assistant MCP server...")
