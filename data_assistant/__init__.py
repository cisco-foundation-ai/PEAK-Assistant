...
async def identify_data_sources(
    ...
    messages = [
        ...
        TextMessage(
            content=f"Splunk server URL: {os.getenv('SPLUNK_SERVER_URL')}\n",
            source="system",
        ),
        TextMessage(
            content=f"Splunk MCP user: {os.getenv('SPLUNK_MCP_USER')}\n",
            source="system",
        ),
        TextMessage(
            content=f"Splunk MCP password: {os.getenv('SPLUNK_MCP_PASSWD')}\n",
            source="system",
        ),
    ]
    ...
    mcp_params = StdioServerParams(command=mcp_command, args=mcp_args)
    ...
    async with McpWorkbench(mcp_params) as mcp:
        ...
        try:
            ...
            return TaskResult(
                messages=[
                    TextMessage(
                        content="An error occurred while identifying data sources.",
                        source="system",
                    )
                ]
            )
