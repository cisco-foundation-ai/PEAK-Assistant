...
def oauth_status():
    ...
    try:
        ...
        for server_name in config_manager.list_servers():
            ...
            if config.auth:
                if config.auth.type == AuthType.OAUTH2_CLIENT_CREDENTIALS:
                    ...
                    user_sessions = (
                        config_manager.user_session_manager.user_sessions.get(
                            user_id, {}
                        )
                    )
        ...
        return jsonify({"user_id": user_id, "servers": servers_status})
    ...
    try:
        ...
        if config.auth.client_registration_url and not config.auth.client_id:
            ...
            registered = loop.run_until_complete(
                config_manager._register_dynamic_client(server_name)
            )
            if not registered:
                flash(
                    f"Failed to dynamically register client for {server_name}. Please check server logs.",
                    "error",
                )
                return redirect(url_for("pages.index"))
        ...
        if dynamic_creds:
            ...
            with OAuth2Session(
                ...
                authorization_url, state = temp_client.create_authorization_url(
    ...
    try:
        ...
        try:
            ...
            if dynamic_creds:
                ...
                logger.info(
                    f"[TOKEN DEBUG] Stored dynamic client credentials for {server_name}"
                )
            else:
                # Standard flow for pre-configured clients
            ...
            logger.info(
                f"OAuth authentication successful for user {user_id}, server {server_name}"
            )
            flash(f"Successfully authenticated with {server_name}", "success")

        except Exception as e:
            logger.error(
                f"OAuth token exchange failed for server {server_name}: {e}",
                exc_info=True,
            )
...
def servers_needing_auth():
    ...
    logger.info(
        f"[Auth Status Check] Checking auth status for user_id: {session.get('user_id')}"
    )
    try:
        user_id = session["user_id"]
        config_manager = get_config_manager()
            ...
            server_details.append(
                {
                    ...
                    "auth_url": url_for(
                        "oauth.initiate_oauth", server_name=server_name
                    ),
                }
            )

        return jsonify(
            {"servers_needing_auth": server_details, "count": len(server_details)}
        )
...
def servers_status():
    ...
    logger.info(
        f"[Server Status Check] Getting status for all servers for user_id: {session.get('user_id')}"
    )
    try:
        _user_id = session["user_id"]
        config_manager = get_config_manager()
        ...
        for server_name, config in sorted(
            config_manager.servers.items(), key=lambda x: x[0].lower()
        ):
            ...
            if requires_auth:
                ...
                if is_authenticated:
                    logger.info(
                        f"[Server Status] {server_name}: Authenticated (token fresh)"
                    )
                else:
                    logger.info(
            ...
            server_info = {
                ...
                "disconnect_url": url_for(
                    "oauth.disconnect_server", server_name=server_name
                )
                if requires_auth
                else None,
                "transport": transport,
        ...
        return jsonify(
            {
                ...
                "authenticated_count": sum(
                    1 for s in all_servers if s["is_authenticated"]
                ),
                "needing_auth_count": sum(
                    1
                    for s in all_servers
                    if s["requires_auth"] and not s["is_authenticated"]
                ),
            }
        )
...
def disconnect_server(server_name):
    ...
    logger.info(
        f"[Disconnect Server] Disconnecting server {server_name} for user_id: {session.get('user_id')}"
    )
    try:
        user_id = session["user_id"]
        config_manager = get_config_manager()
