...
class AuthConfig:
    ...
    discovery_url: Optional[str] = (
        None  # Base URL for OAuth discovery (will append /.well-known/oauth-authorization-server)
    )
    ...
    async def discover_oauth_endpoints(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            well_known_url = urljoin(
                discovery_url.rstrip("/"), "/.well-known/oauth-authorization-server"
            )
            ...
            async with httpx.AsyncClient(
                ...
                if "token_endpoint" not in discovery_data:
                    logger.warning(
                        f"OAuth discovery from {well_known_url} missing required token_endpoint"
                    )
                    return None

                self._discovered_config = discovery_data
                logger.info(
                    f"Successfully discovered OAuth endpoints from {well_known_url}"
                )
    ...
    async def _refresh_token(self) -> str:
        ...
        if self.auth_config.type == AuthType.OAUTH2_CLIENT_CREDENTIALS:
            ...
            if self.refresh_token:
                ...
                raise RuntimeError(
                    "No refresh token available for authorization code flow. User must re-authenticate."
                )
        else:
...
class UserSessionManager:
    ...
    def get_or_create_token_manager(
        self,
        user_id: str,
        server_name: str,
        auth_config: AuthConfig,
        ...
        if server_name not in self.user_sessions[user_id]:
            logger.info(
                f"Creating new OAuth token manager for user {user_id} and server {server_name}"
            )
            self.user_sessions[user_id][server_name] = OAuth2TokenManager(
                auth_config, user_id, server_url
            )
    ...
    def get_user_servers_needing_auth(
        self, user_id: str, servers: Dict[str, MCPServerConfig]
    ) -> List[str]:
        ...
        for server_name, config in servers.items():
            if (
                ...
                and (
                    server_name not in user_session
                    or not user_session[server_name].access_token
                )
            ):
...
class MCPConfigManager:
    ...
    async def _discover_oauth_config(
        ...
        try:
            ...
            logger.info(
                f"Attempting automatic OAuth discovery for {server_name} from: {discovery_url}"
            )
            ...
            async with httpx.AsyncClient(timeout=10) as client:
                ...
                if (
                    "token_endpoint" not in oauth_metadata
                    or "authorization_endpoint" not in oauth_metadata
                ):
                    logger.warning(
                        f"OAuth discovery from {discovery_url} missing required endpoints"
                    )
                    return None
                ...
                logger.info(
                    f"  Authorization endpoint: {oauth_metadata.get('authorization_endpoint')}"
                )
                logger.info(
                    f"  Registration endpoint: {oauth_metadata.get('registration_endpoint', 'None')}"
                )
            ...
            logger.info(
                f"OAuth discovery failed for {server_name} at {server_url}: {e}"
            )
            return None
        ...
        for server_name, server_url in self._servers_needing_oauth_discovery.items():
            try:
                ...
                if auth_config:
                    ...
                    logger.info(
                        f"Updated {server_name} with discovered OAuth configuration"
                    )

                    # Create OAuth manager if needed
                    if auth_config.type == AuthType.OAUTH2_CLIENT_CREDENTIALS:
                        self.oauth_managers[server_name] = OAuth2TokenManager(
                            auth_config, server_url=server_url
                        )
                else:
                    logger.info(
                        f"No OAuth discovered for {server_name}, server will be accessible without authentication"
                    )
    ...
    def __init__(self, config_file: Optional[str] = None):
        logger.info(
            f"[INIT DEBUG] MCPConfigManager.__init__ called with config_file: {config_file}"
        )
        self.config_file = config_file or self._find_config_file()
        logger.info(f"[INIT DEBUG] Using config file path: {self.config_file}")
        logger.info(
            f"[INIT DEBUG] Config file exists: {os.path.exists(self.config_file)}"
        )

        self.servers: Dict[str, MCPServerConfig] = {}
        self.server_groups: Dict[str, List[str]] = {}
        self.oauth_managers: Dict[
            str, OAuth2TokenManager
        ] = {}  # For client credentials
        self.user_session_manager = UserSessionManager()
        self._servers_needing_oauth_discovery: Dict[
            str, str
        ...
        self._load_config()
        logger.info(
            f"[INIT DEBUG] _load_config() completed. Loaded {len(self.servers)} servers and {len(self.server_groups)} server groups"
        )

        # Perform OAuth discovery for servers that need it
        if self._servers_needing_oauth_discovery:
            logger.info(
                f"[INIT DEBUG] Performing OAuth discovery for {len(self._servers_needing_oauth_discovery)} servers"
            )
            ...
            try:
                ...
                if loop.is_running():
                    ...
                    logger.info(
                        f"[INIT DEBUG] OAuth discovery completed using separate thread for {len(self._servers_needing_oauth_discovery)} servers"
                    )
                else:
                    # Event loop exists but not running, can use run_until_complete
                    loop.run_until_complete(self._perform_oauth_discovery())
                    logger.info(
                        f"[INIT DEBUG] OAuth discovery completed using run_until_complete for {len(self._servers_needing_oauth_discovery)} servers"
                    )
                ...
                if "no running event loop" in str(e) or "no current event loop" in str(
                    ...
                    try:
                        ...
                        logger.info(
                            f"[INIT DEBUG] OAuth discovery completed using asyncio.run for {len(self._servers_needing_oauth_discovery)} servers"
                        )
                    except Exception as ex:
                        logger.warning(
                            f"[INIT DEBUG] Failed to run OAuth discovery with asyncio.run: {ex}. Discovery will happen on first access."
                        )
                else:
                    logger.warning(
                        f"[INIT DEBUG] Failed to run OAuth discovery synchronously: {e}. Discovery will happen on first access."
                    )
            except Exception as ex:
                logger.warning(
                    f"[INIT DEBUG] Failed to run OAuth discovery synchronously: {ex}. Discovery will happen on first access."
                )
    ...
    def _load_config(self):
        ...
        logger.info(
            f"[LOAD CONFIG DEBUG] Starting _load_config for file: {self.config_file}"
        )

        if not os.path.exists(self.config_file):
            logger.error(
                f"[LOAD CONFIG DEBUG] MCP config file not found: {self.config_file}"
            )
            logger.error(
                f"[LOAD CONFIG DEBUG] Current working directory: {os.getcwd()}"
            )
            logger.error(
                "[LOAD CONFIG DEBUG] File existence check failed - configuration loading aborted"
            )
            return

        logger.info(
            "[LOAD CONFIG DEBUG] Config file exists, attempting to read and parse JSON"
        )
        try:
            with open(self.config_file, "r") as f:
                config_data = json.load(f)
            logger.info(
                f"[LOAD CONFIG DEBUG] JSON parsing successful, config_data type: {type(config_data)}"
            )
            logger.info(
                f"[LOAD CONFIG DEBUG] Top-level keys in config: {list(config_data.keys()) if isinstance(config_data, dict) else 'Not a dict'}"
            )
            ...
            for server_config in servers_to_load:
                ...
                if "auth" in server_config:
                    ...
                    auth_config = AuthConfig(
                        ...
                        client_registration_url=auth_data.get(
                ...
                elif server_url and transport in [
                    TransportType.HTTP,
                    TransportType.SSE,
                ]:
                    # No explicit auth config - attempt automatic OAuth discovery
                    logger.info(
                        f"No auth config for {name}, attempting automatic OAuth discovery from {server_url}"
                    )
                    try:
                        ...
                        logger.warning(
                            f"Failed to schedule OAuth discovery for {name}: {e}"
                        )
                        auth_config = None
                ...
                if (
                    auth_config
                    and auth_config.type == AuthType.OAUTH2_CLIENT_CREDENTIALS
                ):
                    self.oauth_managers[name] = OAuth2TokenManager(
                        auth_config, server_url=server_config.get("url")
                    )
                elif (
                    auth_config
                    and auth_config.type == AuthType.OAUTH2_AUTHORIZATION_CODE
                ):
            ...
            logger.info(
                f"[CONFIG DEBUG] Loaded server groups: {list(self.server_groups.keys())}"
            )
            logger.info(
                f"[CONFIG DEBUG] Research group contents: {self.server_groups.get('research', 'NOT FOUND')}"
            )
    ...
    def get_server_group(self, group_name: str) -> List[str]:
        ...
        logger.info(
            f"[RUNTIME DEBUG] Available server groups: {list(self.server_groups.keys())}"
        )
    ...
    def _save_config(self):
        ...
        config_data = {"mcpServers": {}, "serverGroups": self.server_groups}
        ...
        for name, server_config in self.servers.items():
            ...
            if server_config.auth:
                ...
                for field in [
                    ...
                    if (
                        hasattr(server_config.auth, field)
                        and getattr(server_config.auth, field) is not None
                    ):
        ...
        try:
            ...
            logger.info(
                f"Successfully saved updated configuration to {self.config_file}"
            )
        except Exception as e:
    ...
    async def _register_dynamic_client(self, server_name: str) -> bool:
        ...
        if not redirect_uri:
            try:
                redirect_uri = url_for("oauth.oauth_callback", _external=True)
                logger.info(
                    f"Auto-generated redirect URI for {server_name}: {redirect_uri}"
                )
            except Exception:
                # Fallback if Flask context not available
                redirect_uri = "http://localhost:5000/oauth/callback"
                logger.info(
                    f"Using fallback redirect URI for {server_name}: {redirect_uri}"
                )
        ...
        logger.info(
            f"Attempting dynamic registration for {server_name} at {config.auth.client_registration_url}"
        )
        async with httpx.AsyncClient() as client:
            try:
                response = await client.post(
                    config.auth.client_registration_url, json=registration_payload
                )
                response.raise_for_status()
                ...
                logger.info(
                    f"Successfully registered client for {server_name}. Client ID: {config.auth.client_id}"
                )
                logger.info(
                    "Dynamic credentials stored in memory only - configuration file remains unchanged"
                )
                ...
                logger.error(
                    f"Failed to dynamically register client for {server_name}: {e.response.status_code} - {e.response.text}"
                )
                return False
            except Exception as e:
                logger.error(
                    f"An unexpected error occurred during dynamic registration for {server_name}: {e}"
                )
                return False
...
class MCPClientManager:
    ...
    def __init__(self, config_manager: "MCPConfigManager"):
        ...
        self.user_workbenches: Dict[
            str, Dict[str, McpWorkbench]
        ] = {}  # User-specific workbenches
    ...
    async def connect_server(
        self, server_name: str, user_id: Optional[str] = None
    ) -> bool:
    ...
    async def _connect_stdio_server(
        self, server_name: str, config: MCPServerConfig, user_id: Optional[str] = None
    ) -> bool:
        ...
        if config.env:
            ...
            logger.info(
                f"Setting environment variables for {server_name}: {list(config.env.keys())}"
            )
            for key, value in config.env.items():
                logger.debug(
                    f"  {key}={'*' * len(value) if 'PASS' in key.upper() else value}"
                )
        ...
        server_params = StdioServerParams(
            command=config.command, args=args_list, env=env
        )
        logger.debug(
            f"Created StdioServerParams with command: {config.command}, args: {args_list}, env keys: {list(env.keys()) if env else 'None'}"
        )
        ...
        logger.info(
            f"Connected to stdio server: {server_name}"
    ...
    async def _connect_http_server(
        ...
        logger.info(
            f"Connected to HTTP server: {server_name}"
            + (f" for user {user_id}" if user_id else "")
        )
        return True

    async def _get_auth_headers(
        self, config: MCPServerConfig, user_id: Optional[str] = None
    ) -> Dict[str, str]:
    ...
    async def _connect_sse_server(
        ...
        logger.info(
            f"[SSE DEBUG] Starting connection to {server_name} for user_id: {user_id}"
        )
        ...
        logger.info(
            f"[SSE DEBUG] Auth headers obtained for {server_name}: {list(headers.keys()) if headers else 'None'}"
        )
        ...
        try:
            ...
            logger.info(
                f"[SSE DEBUG] SseServerParams created successfully for {server_name}"
            )
            ...
            await workbench.__aenter__()
            logger.info(
                f"[SSE DEBUG] Workbench context entered successfully for {server_name}"
            )

            # Test connection health by listing tools
            try:
                logger.info(
                    f"[SSE DEBUG] Testing connection by listing tools for {server_name}"
                )
                tools = await workbench.list_tools()
                logger.info(
                    f"[SSE DEBUG] SSE workbench for {server_name} connected successfully with {len(tools)} tools available"
                )
            except Exception as e:
                logger.warning(
                    f"[SSE DEBUG] SSE workbench for {server_name} connection test failed: {e}"
                )
                # Try to continue anyway

            # Store workbench (user-specific or system-level)
            logger.info(
                f"[SSE DEBUG] Storing workbench for {server_name}, user_id: {user_id}"
            )
            if user_id:
                if user_id not in self.user_workbenches:
                    self.user_workbenches[user_id] = {}
                    logger.info(
                        f"[SSE DEBUG] Created new user_workbenches entry for user {user_id}"
                    )
                self.user_workbenches[user_id][server_name] = workbench
                logger.info(
                    f"[SSE DEBUG] Stored user-specific workbench for {server_name} under user {user_id}"
                )
                logger.info(
                    f"[SSE DEBUG] Current user_workbenches keys: {list(self.user_workbenches.keys())}"
                )
                logger.info(
                    f"[SSE DEBUG] Workbenches for user {user_id}: {list(self.user_workbenches[user_id].keys())}"
                )
            else:
                self.workbenches[server_name] = workbench
                logger.info(
                    f"[SSE DEBUG] Stored system-level workbench for {server_name}"
                )
                logger.info(
                    f"[SSE DEBUG] Current system workbenches: {list(self.workbenches.keys())}"
                )
    ...
    async def connect_server_group(
            ...
            if client_info.get("type") == "sse":
                ...
                if context_manager and hasattr(context_manager, "__aexit__"):
                    try:
                        ...
                        logger.warning(
                            f"Error closing SSE context manager for {server_name}: {e}"
                        )
...
async def setup_mcp_servers(
    server_group: str = "all", user_id: Optional[str] = None
) -> List[str]:
