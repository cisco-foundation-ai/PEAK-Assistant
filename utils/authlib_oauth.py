...
class AuthlibOAuthManager:
    ...
    def update_token(self, name: str, token: dict, refresh_token: str = None, access_token: str = None):
        ...
        logger.info(f"[SESSION DEBUG] Session has oauth_tokens: {'oauth_tokens' in session}")
        if 'oauth_tokens' in session:
            logger.info(f"[SESSION DEBUG] Current session tokens: {list(session['oauth_tokens'].keys())}")
        ...
        logger.info(f"[SESSION DEBUG] Session has oauth_tokens: {'oauth_tokens' in session}")
        if 'oauth_tokens' in session:
            logger.info(f"[SESSION DEBUG] Session tokens after store: {list(session['oauth_tokens'].keys())}")
            if name in session['oauth_tokens']:
                stored_token = OAuth2Token(session['oauth_tokens'][name])
                logger.info(f"[SESSION DEBUG] Stored token expires_at: {stored_token.get('expires_at')} ({'expired' if stored_token.is_expired() else 'valid'})")
        ...
        try:
            ...
            if auth_config.discovery_url or (auth_config.enable_discovery and server_url):
                ...
                if discovery_url:
                    ...
                    logger.info(f"Registering OAuth client for {server_name} with discovery: {metadata_url}")
            ...
            if not discovery_url or not auth_config.discovery_url and not (auth_config.enable_discovery and server_url):
    ...
    def store_token(self, server_name: str, token: OAuth2Token):
        ...
        logger.info(f"[TOKEN DEBUG] Token expires_at: {token.get('expires_at')} ({'expired' if token.is_expired() else 'valid'})")
    ...
    def get_auth_headers(self, server_name: str) -> Dict[str, str]:
        ...
        if token.is_expired():
            logger.info(f"[TOKEN DEBUG] Token for {server_name} is expired, attempting refresh")
            if self._refresh_token_manually(server_name, token):
                # Get the refreshed token
                token = self.get_token(server_name)
                if not token:
                    logger.error(f"[TOKEN DEBUG] Failed to get refreshed token for {server_name}")
                    return {}
            else:
    ...
    def _refresh_token_manually(self, server_name: str, token: OAuth2Token) -> bool:
        ...
        try:
            ...
            if not client_id or not client_secret:
                logger.warning(f"[TOKEN DEBUG] No client credentials available for {server_name} - skipping refresh")
                logger.warning(f"[TOKEN DEBUG] Checked dynamic credentials: {bool(token.get('_dynamic_client_id'))}, static credentials: {bool(client.client_id)}")
                return False
            
            logger.info(f"[TOKEN DEBUG] Using {'dynamic' if token.get('_dynamic_client_id') else 'static'} client credentials for {server_name}")
            ...
            if not token_endpoint:
                if hasattr(client, 'server_metadata') and client.server_metadata:
                    ...
                    logger.error(f"[TOKEN DEBUG] No server metadata available for {server_name}")
                    return False
            ...
            logger.info(f"[TOKEN DEBUG] Using {'dynamic' if token.get('_dynamic_token_url') else 'metadata'} token endpoint: {token_endpoint}")
            ...
            if not refresh_token:
                logger.error(f"[TOKEN DEBUG] No refresh token available for {server_name}")
                return False
            ...
            logger.info(f"[TOKEN DEBUG] Attempting token refresh for {server_name} at {token_endpoint}")
            ...
            if response.status_code == 200:
                ...
                logger.info(f"[TOKEN DEBUG] Old token expires_at: {token.get('expires_at')}")
                logger.info(f"[TOKEN DEBUG] New token data keys: {list(new_token_data.keys())}")
                logger.info(f"[TOKEN DEBUG] New token expires_in: {new_token_data.get('expires_in')}")
                logger.info(f"[TOKEN DEBUG] New token expires_at: {new_token_data.get('expires_at')}")
                ...
                if 'expires_in' in new_token_data and 'expires_at' not in new_token_data:
                    import time
                    updated_token['expires_at'] = int(time.time()) + int(new_token_data['expires_in'])
                    logger.info(f"[TOKEN DEBUG] Calculated expires_at: {updated_token['expires_at']}")
                
                logger.info(f"[TOKEN DEBUG] Final updated token expires_at: {updated_token.get('expires_at')}")
                ...
                logger.error(f"[TOKEN DEBUG] Token refresh failed for {server_name}: HTTP {response.status_code}")
                logger.error(f"[TOKEN DEBUG] Response: {response.text}")
                return False
                
        except Exception as e:
            logger.error(f"[TOKEN DEBUG] Exception during token refresh for {server_name}: {e}")
            return False
    ...
    def _get_health_endpoint(self, server_name: str) -> Optional[str]:
        ...
        try:
            ...
            if not base_url:
                logger.warning(f"[TOKEN DEBUG] Could not determine base URL for {server_name}")
                return None
            ...
            logger.warning(f"[TOKEN DEBUG] Error getting health endpoint for {server_name}: {e}")
            return None
    ...
    def _get_authenticated_endpoint(self, server_name: str) -> Optional[str]:
        ...
        try:
            ...
            logger.warning(f"[TOKEN DEBUG] Error getting authenticated endpoint for {server_name}: {e}")
            return None
    ...
    def _trigger_oauth_refresh(self, server_name: str) -> bool:
        ...
        try:
            client = self.get_client(server_name)
            if not client:
                logger.debug(f"[TOKEN DEBUG] No OAuth client available for {server_name}")
                return False
            ...
            if not auth_endpoint:
                logger.debug(f"[TOKEN DEBUG] No authenticated endpoint available for {server_name}")
                return False
            
            logger.info(f"[TOKEN DEBUG] Triggering OAuth refresh for {server_name} via {auth_endpoint}")
            ...
            token_refreshed = (old_access_token != new_access_token) and new_access_token is not None
            
            if token_refreshed:
                logger.info(f"[TOKEN DEBUG] OAuth client successfully refreshed token for {server_name}")
                return True
            elif response.status_code < 400:
                logger.info(f"[TOKEN DEBUG] OAuth request succeeded for {server_name} (HTTP {response.status_code}) - token still valid")
                return True
            else:
                logger.warning(f"[TOKEN DEBUG] OAuth client request failed for {server_name} (HTTP {response.status_code})")
                return False
            
        except Exception as e:
            logger.info(f"[TOKEN DEBUG] OAuth client refresh trigger failed for {server_name}: {e}")
            return False
    ...
    def _is_dynamic_client_registration(self, server_name: str) -> bool:
        ...
        try:
            ...
            has_static_credentials = (
                hasattr(client, 'client_id') and client.client_id and
                hasattr(client, 'client_secret') and client.client_secret
            )
            
            if not has_static_credentials:
                logger.info(f"[TOKEN DEBUG] {server_name} appears to use dynamic client registration (no static credentials)")
                return True
            
            return False
            
        except Exception as e:
            logger.warning(f"[TOKEN DEBUG] Error checking client registration type for {server_name}: {e}")
            return False
    ...
    def get_fresh_token(self, server_name: str) -> Optional[OAuth2Token]:
        ...
        if self._trigger_oauth_refresh(server_name):
            ...
            if refreshed_token and not refreshed_token.is_expired():
                logger.info(f"[TOKEN DEBUG] Successfully refreshed token for {server_name} via OAuth client")
                return refreshed_token
            else:
                logger.warning(f"[TOKEN DEBUG] OAuth client refresh didn't produce valid token for {server_name}")
        ...
        if self._refresh_token_manually(server_name, stored_token):
            ...
            refreshed_token = self.get_token(server_name)
            if refreshed_token:
                logger.info(f"[TOKEN DEBUG] Successfully refreshed token for {server_name} via manual refresh")
                return refreshed_token
        
        # Strategy 3: Return expired token (let caller handle auth failure)
        logger.warning(f"[TOKEN DEBUG] All refresh strategies failed for {server_name}, returning expired token")
        return stored_token
    ...
    def get_fresh_auth_headers(self, server_name: str) -> Dict[str, str]:
        ...
        if not token:
            ...
            if self._is_dynamic_client_registration(server_name):
                logger.error(f"[TOKEN DEBUG] {server_name} requires re-authentication (dynamic client registration)")
                logger.error(f"[TOKEN DEBUG] Please visit the OAuth flow to re-authenticate with {server_name}")
            else:
