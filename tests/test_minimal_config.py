...
async def test_minimal_config():
    ...
    if effective_discovery_url == expected_discovery_url:
        ...
        print(f"❌ Discovery URL derivation failed. Expected: {expected_discovery_url}, Got: {effective_discovery_url}")
    ...
    if effective_redirect_uri == expected_fallback_redirect:
        ...
        print(f"❌ Redirect URI generation failed. Expected: {expected_fallback_redirect}, Got: {effective_redirect_uri}")
    ...
    try:
        ...
        if discovered_config:
            ...
            print(f"   Authorization endpoint: {discovered_config.get('authorization_endpoint')}")
        else:
        ...
        try:
            ...
            print(f"❌ Unexpected success - should have failed without manual token URL")
...
async def test_minimal_config_with_manual_fallback():
    ...
    try:
        ...
        if "client_id=qu41R1ZW5XWteQwE" in full_auth_url and "localhost:8000/oauth/callback" in full_auth_url:
            print("✅ Authorization URL contains correct client_id and auto-generated redirect_uri")
        else:
...
async def test_configuration_field_count():
    ...
    old_fields = [
        "type", "requires_user_auth", "client_id", "client_secret", "scope",
        "token_url", "authorization_url", "redirect_uri", "client_registration_url", "header_name"
    ]
