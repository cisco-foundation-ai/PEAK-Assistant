...
REQUIRED_SECTIONS = [
    "Overview", "Threat Actors", "Technique Details", "Detection", 
    "Typical Datasets", "Published Hunts", "Previous Hunting Information",
    "Commonly-Used Tools", "References", "Other Information"
]
...
class ReportEvaluator:
    ...
    def __init__(self, api_key: str, verbose: bool = False, quiet: bool = False, 
                 log_file: str = None, json_output_file: str = None, cheap_mode: bool = False):
    ...
    def evaluate_with_llm_retry(self, prompt: str, metric_name: str, model: str, 
                                max_retries: int = 2, max_tokens: int = 500) -> Optional[Dict]:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    if self.verbose:
                        self.print_output(f"    Retrying {metric_name} (attempt {attempt + 2}/{max_retries + 1})...")
                    # Make instructions even stronger for retry
                    full_prompt = prompt + "\n\nRETRY: Previous response was not valid JSON. Return ONLY a JSON object like {\"score\": 50, \"feedback\": \"example\"}" + json_instructions
                else:
                    if self.verbose:
                        self.print_output(f"    ⚠️ Failed to get valid JSON for {metric_name} after {max_retries + 1} attempts")
                    
            except Exception as e:
                if self.verbose and attempt == max_retries:
                    self.print_output(f"    LLM API error for {metric_name}: {str(e)[:50]}")
        
        return None
    
    def extract_section_with_llm(self, report: str, section_name: str, section_description: str, 
        ...
        if variations is None:
            variations = []
        variations_str = ", ".join([f'"{v}"' for v in variations]) if variations else f'variations of "{section_name}"'
...
Return the extracted section content:"""
        ...
        try:
            ...
            if self.verbose:
                self.print_output(f"    Failed to extract section {section_name}: {str(e)[:50]}")
    ...
    def detect_outliers(self, scores: List[float], backend_name: str = None) -> List[Tuple[int, float]]:
    ...
    def calculate_trimmed_mean(self, scores: List[float], trim_percent: float = 0.1) -> float:
    ...
    def format_statistical_summary(self, scores: List[float], backend_name: str, 
                                  topic_score_map: Dict[str, float] = None) -> Dict:
        """Generate comprehensive statistical summary for a set of scores"""
        if not scores:
            return {
                'mean': 0, 'median': 0, 'trimmed_mean': 0,
                'consistency': 0, 'confidence_interval': (0, 0),
                'outliers': []
            }
    ...
    def evaluate_structure_compliance(self, report: str, topic: str = None) -> MetricResult:
        ...
        for section in REQUIRED_SECTIONS:
            ...
            if re.search(pattern, report, re.MULTILINE | re.IGNORECASE):
                ...
                match = re.search(section_pattern, report, re.MULTILINE | re.IGNORECASE | re.DOTALL)
                if match:
    ...
    def evaluate_technical_depth(self, report: str, topic: str = None) -> MetricResult:
...
{{
    ...
    "confidence": 0.9
        ...
        if result:
            ...
            return MetricResult(score=score, feedback="Fallback: counted code blocks only", confidence=0.3)
    
    def evaluate_technical_accuracy(self, report: str, topic: str = None) -> MetricResult:
...
{{
    ...
    "confidence": 0.85
        ...
        result = self.evaluate_with_llm_retry(prompt, "technical_accuracy", self.model_critical, max_tokens=800)
    ...
    def evaluate_mitre_coverage(self, report: str, topic: str = None) -> MetricResult:
...
{{
    ...
    "confidence": 0.8
        ...
        if result:
            ...
            return MetricResult(score=score, feedback=f"Found {len(mitre_ids)} MITRE IDs", confidence=0.3)
    
    def evaluate_detection_quality(self, report: str, topic: str = None) -> MetricResult:
        ...
        detection_content = self.extract_section_with_llm(
            ...
            variations=["Detections", "Detection Methods", "Detection Strategies", "How to Detect", "Detection Rules"]
        )
        ...
        if not detection_content:
            ...
            match = re.search(detection_pattern, report, re.MULTILINE | re.IGNORECASE | re.DOTALL)
...
{{
    ...
    "confidence": 0.9
        ...
        if result:
            ...
            # Fallback
            has_queries = bool(re.search(r'(index=|SELECT|EventID|rule:|detection:)', detection_content))
            score = 50 if detection_content else 0
            if has_queries:
                score += 30
            return MetricResult(score=score, feedback="Detection section evaluated", confidence=0.3)
    
    def evaluate_dataset_documentation(self, report: str, topic: str = None) -> MetricResult:
        ...
        dataset_content = self.extract_section_with_llm(
            ...
            variations=["Datasets", "Required Datasets", "Data Sources", "Log Sources", "Required Data", "Hunt Datasets"]
        )
        
        # If no content found, try fallback regex
        if not dataset_content:
            dataset_pattern = r'^##?\s+Typical Datasets\s*\n(.*?)(?=^##?\s+|\Z)'
            match = re.search(dataset_pattern, report, re.MULTILINE | re.IGNORECASE | re.DOTALL)
...
{{
    ...
    "confidence": 0.9
        ...
        result = self.evaluate_with_llm_retry(prompt, "dataset_documentation", self.model_quality)
        ...
        if result:
            ...
            return MetricResult(score=score, feedback="Dataset section present", confidence=0.3)
    
    def evaluate_threat_actor_specificity(self, report: str, topic: str = None) -> MetricResult:
        ...
        ta_content = self.extract_section_with_llm(
            ...
            variations=["Threat Actor Groups", "Known Threat Actors", "Adversaries", "APT Groups", "Threat Groups", "Actors"]
        )
        
        # If no content found, try fallback regex
        if not ta_content:
            ta_pattern = r'^##?\s+Threat Actors\s*\n(.*?)(?=^##?\s+|\Z)'
            match = re.search(ta_pattern, report, re.MULTILINE | re.IGNORECASE | re.DOTALL)
...
{{
    ...
    "confidence": 0.8
        ...
        result = self.evaluate_with_llm_retry(prompt, "threat_actor_specificity", self.model_fast)
        ...
        if result:
            ...
            return MetricResult(score=score, feedback="Threat actor section evaluated", confidence=0.3)
    
    def evaluate_reference_quality(self, report: str, topic: str = None) -> MetricResult:
        ...
        ref_content = self.extract_section_with_llm(
            ...
            variations=["Sources", "Citations", "External References", "Bibliography", "Links", "Further Reading"]
        )
        
        # If no content found, try fallback regex
        if not ref_content:
            ref_pattern = r'^##?\s+References\s*\n(.*?)(?=^##?\s+|\Z)'
            match = re.search(ref_pattern, report, re.MULTILINE | re.IGNORECASE | re.DOTALL)
        ...
        details = {
            ...
            'has_vendor_docs': any(d for d in domains if 'microsoft.com' in d or 'docs.' in d),
        ...
        feedback = f"Found {len(urls)} references from {len(unique_domains)} unique domains"
        
        return MetricResult(score=score, details=details, feedback=feedback)
    
    async def evaluate_url_validity_async(self, report: str, topic: str = None) -> MetricResult:
    ...
    def evaluate_url_validity(self, report: str, topic: str = None) -> MetricResult:
    ...
    def evaluate_log_example_quality(self, report: str, topic: str = None) -> MetricResult:
...
{{
    ...
    "confidence": 0.9
        ...
        result = self.evaluate_with_llm_retry(prompt, "log_example_quality", self.model_quality)
        ...
        if result:
            ...
            return MetricResult(score=score, feedback=f"Found {len(code_blocks)} code blocks", confidence=0.3)
    
    def evaluate_instruction_clarity(self, report: str, topic: str = None) -> MetricResult:
        ...
        tech_content = self.extract_section_with_llm(
            ...
            variations=["Technical Details", "How It Works", "Technique Description", "Attack Methodology", 
                        "Technical Implementation", "Detailed Description", "How the Attack Works"]
        )
        
        # If no content found, try fallback regex
        if not tech_content:
            tech_pattern = r'^##?\s+Technique Details\s*\n(.*?)(?=^##?\s+|\Z)'
            match = re.search(tech_pattern, report, re.MULTILINE | re.IGNORECASE | re.DOTALL)
...
{{
    ...
    "confidence": 0.9
        ...
        result = self.evaluate_with_llm_retry(prompt, "instruction_clarity", self.model_quality)
        ...
        if result:
            ...
            has_steps = bool(re.search(r'(step \d|first|then|next|finally|\d\.)', tech_content, re.IGNORECASE))
            score = 60 if has_steps else 30
            return MetricResult(score=score, feedback="Instruction clarity evaluated", confidence=0.3)
    
    def evaluate_cross_section_consistency(self, report: str, topic: str = None) -> MetricResult:
...
{{
    ...
    "confidence": 0.85
        ...
        result = self.evaluate_with_llm_retry(prompt, "cross_section_consistency", self.model_quality)
        ...
        if result:
            ...
            return MetricResult(score=70, feedback="Could not evaluate consistency", confidence=0.2)
    
    def evaluate_tool_documentation(self, report: str, topic: str = None) -> MetricResult:
        ...
        tools_content = self.extract_section_with_llm(
            ...
            variations=["Common Tools", "Tools", "Attacker Tools", "Tools Used", "Attack Tools", 
                        "Malware and Tools", "Software Used"]
        )
        
        # If no content found, try fallback regex
        if not tools_content:
            tools_pattern = r'^##?\s+Commonly-Used Tools\s*\n(.*?)(?=^##?\s+|\Z)'
            match = re.search(tools_pattern, report, re.MULTILINE | re.IGNORECASE | re.DOTALL)
        ...
        tool_lines = [line for line in tools_content.split('\n') if line.strip() and not line.strip().lower() == 'n/a']
        ...
        return MetricResult(
            score=score,
            details={'tool_count': len(tool_lines)},
            feedback=feedback
        )
    ...
    def evaluate_report(self, report_data: Dict) -> ReportMetrics:
        ...
        if self.verbose:
            ...
            self.print_output(f"  {'Metric':<30} {'Score':>8} {'Weight':>8} {'Feedback'}")
        ...
        for metric_name, (metric_func, weight) in self.metric_functions.items():
            try:
                ...
                if self.verbose:
                    ...
                    feedback_str = result.feedback[:40] + "..." if len(result.feedback) > 40 else result.feedback
                    self.print_output(f"  {metric_name:<30} {score_str:>8} {weight_str:>8} {feedback_str}")
                ...
                if self.verbose:
                    self.print_output(f"  {metric_name:<30} {'ERROR':>8} {f'x{weight:.1f}':>8} Failed: {str(e)[:35]}...")
                else:
    ...
    def compare_reports(self, old_metrics: ReportMetrics, new_metrics: ReportMetrics) -> ComparisonResult:
        ...
        for metric_name in self.metric_functions.keys():
            old_result = old_metrics.metric_results.get(metric_name, MetricResult(score=0))
            new_result = new_metrics.metric_results.get(metric_name, MetricResult(score=0))
            ...
            if abs(diff) > 10:  # Significant difference
                if diff > 0:
                    key_differences.append(f"{metric_name}: NEW better by {diff:.1f} points")
                else:
                    key_differences.append(f"{metric_name}: OLD better by {abs(diff):.1f} points")
    ...
    def process_reports(self, input_file: str, output_file: str):
        ...
        with open(input_file, 'r') as f:
            for line_num, line in enumerate(f, 1):
                try:
                    ...
                    if data.get('encoded') or self._looks_like_base64(data.get('report', '')):
                        try:
                            report_bytes = base64.b64decode(data['report'])
                            data['report'] = report_bytes.decode('utf-8')
                        except Exception as e:
                            self.print_output(f"Warning: Failed to decode report on line {line_num}: {e}")
                            continue
        ...
        if all(b == 1 for b in backends_per_topic):
            ...
            self.print_output(f"Detected single evaluation mode")
            ...
            self.print_output(f"Detected comparison mode: {len(unique_backends)} backends ({', '.join(sorted(unique_backends))})")
            ...
            self.print_output("Warning: Mixed mode detected - some topics have different numbers of backends")
    ...
    def _process_single_mode(self, all_reports: List[Dict], output_file: str):
        ...
        for report_data in all_reports:
            ...
            output_data = {
                ...
                'metric_scores': {k: v.score for k, v in metrics.metric_results.items()}
            }
    ...
    def _process_comparison_mode(self, all_reports: List[Dict], output_file: str):
        ...
        for topic, reports in reports_by_topic.items():
            ...
            for report in reports:
                ...
                for metric_name, result in metrics.metric_results.items():
                    metric_scores_by_backend[metrics.backend][metric_name].append(result.score)
            ...
            output_data = {
                ...
                'backends': {
                    m.backend: {
                        ...
                        'metrics': {k: {'score': v.score, 'feedback': v.feedback} 
        ...
        self._display_overall_comparison_summary_enhanced(
            backend_wins, backend_scores, backend_rankings, 
            metric_scores_by_backend, backend_topic_scores, len(reports_by_topic)
        )
    ...
    def _display_topic_comparison(self, topic: str, metrics_list: List[ReportMetrics]):
        ...
        for i, metrics in enumerate(metrics_list):
            trophy = " 🏆" if i == 0 else ""
            self.print_output(f"  {i+1}. {metrics.backend} ({metrics.total_score:.1f}){trophy}")
        ...
        if len(metrics_list) > 1:
            ...
            for metrics in metrics_list[1:]:
                ...
                for metric, diff in better_metrics[:2]:
                    self.print_output(f"    • {metric}: {metrics.backend} better by {diff:.1f} points")
                for metric, diff in worse_metrics[:2]:
                    self.print_output(f"    • {metric}: {winner.backend} better by {diff:.1f} points")
                if len(roughly_equal) > 0 and len(roughly_equal) <= 3:
                    self.print_output(f"    • roughly equal: {', '.join(roughly_equal)}")
                elif len(roughly_equal) > 3:
                    self.print_output(f"    • roughly equal: {', '.join(roughly_equal[:3])} and {len(roughly_equal)-3} more")
        ...
        for metric_name in self.metric_functions.keys():
            ...
            self.print_output(f"  ⭐ {metric_name}: {best_backend} ({best_score:.1f}) → {', '.join(others)}")
        ...
        self.print_output(f"\nKey Insights:")
        for backend, count in sorted(backend_metric_wins.items(), key=lambda x: x[1], reverse=True):
            self.print_output(f"  • {backend} dominates {count}/{len(self.metric_functions)} metrics")
        ...
        if biggest_gap_metric:
            self.print_output(f"  • Biggest gap: {biggest_gap_metric} ({biggest_gap:.1f} points)")
        ...
        for metric_name in self.metric_functions.keys():
            scores_with_backend = [(m.backend, m.metric_results[metric_name].score) for m in metrics_list]
            ...
            for i in range(len(scores_with_backend)):
                for j in range(i+1, len(scores_with_backend)):
                    ...
                    if diff > 15:
                        self.print_output(f"  • {metric_name}: {scores_with_backend[i][0]} leads {scores_with_backend[j][0]} by {diff:.1f}")
                        notable_found = True
        
        if not notable_found:
            self.print_output(f"  • No differences greater than 15 points")
    
    def _display_overall_comparison_summary_enhanced(self, backend_wins, backend_scores, 
                                                    backend_rankings, metric_scores_by_backend,
                                                    backend_topic_scores, total_topics):
        ...
        for i, stats in enumerate(backend_stats):
            ...
            self.print_output(f"  {i+1}. {stats['backend']}: {standing} "
            ...
            if stats['outliers']:
                for outlier in stats['outliers']:
                    self.print_output(f"     └─ Outlier: score of {outlier['score']:.1f} on '{outlier['topic']}' "
        ...
        for stats in backend_stats:
            ...
            self.print_output(f"    Mean: {stats['mean']:.1f}, Median: {stats['median']:.1f}, "
                            f"Trimmed Mean (10%): {stats['trimmed_mean']:.1f}")
            self.print_output(f"    95% Confidence Interval: [{ci_low:.1f}, {ci_high:.1f}]")
        ...
        if has_any_outliers:
            ...
            self.print_output(f"   Outliers may indicate LLM hallucinations or exceptional cases.")
        ...
        for metric_name in self.metric_functions.keys():
            ...
            if metric_avgs:
                ...
                self.print_output(f"  ⭐ {metric_name}: {champion[0]} ({measure} {champion[1]:.1f})")
            ...
            for metric_name in self.metric_functions.keys():
                ...
                for backend in backend_scores.keys():
                    if metric_name in metric_scores_by_backend[backend]:
                        ...
                        if has_any_outliers:
                            backend_avgs[backend] = self.calculate_median(metric_scores_by_backend[backend][metric_name])
                        else:
                            backend_avgs[backend] = statistics.mean(metric_scores_by_backend[backend][metric_name])
                
                if len(backend_avgs) > 1:
                    sorted_backends = sorted(backend_avgs.items(), key=lambda x: x[1], reverse=True)
                    ...
                    if avg_gap > 10:  # Significant consistent difference
                        self.print_output(f"  • {best_backend} consistently beats others on {metric_name} (avg +{avg_gap:.1f})")
        
        # Reliability Assessment
        self.print_output(f"\nReliability Assessment:")
        for stats in backend_stats:
            reliability = "High" if stats['consistency'] > 85 else "Medium" if stats['consistency'] > 70 else "Low"
            ci_width = stats['confidence_interval'][1] - stats['confidence_interval'][0]
            precision = "High" if ci_width < 10 else "Medium" if ci_width < 20 else "Low"
            
            self.print_output(f"  {stats['backend']}: Reliability: {reliability}, Precision: {precision}")
            
            if stats['has_outliers']:
                self.print_output(f"    ⚠️ Contains outliers - consider investigating anomalous reports")

def main():
    parser = argparse.ArgumentParser(description='Evaluate and compare threat hunting reports')
    ...
    parser.add_argument('--no-json',
                       action='store_true',
                       help='Disable full JSON output file')
