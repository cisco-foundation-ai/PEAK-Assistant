    ...
    try:
        ...
        if "github-oauth-server" in servers:
            ...
            if server_config.auth:
                ...
                print(f"   Registration URL: {server_config.auth.client_registration_url}")
...
async def test_oauth_discovery_with_real_server():
    ...
    try:
        print("🔍 Testing against real server (requires server running on localhost:8788)")
        ...
        if "real-oauth-test" in servers:
            ...
            if server.auth:
                ...
                if server.auth.client_registration_url:
                    print(f"   Registration endpoint available: {server.auth.client_registration_url}")
...
async def test_configuration_comparison():
    ...
    print(f"   Fields required: {len(list(old_config['mcpServers']['oauth-server'].keys())) + len(list(old_config['mcpServers']['oauth-server']['auth'].keys()))}")
    ...
    reduction = ((len(list(old_config['mcpServers']['oauth-server'].keys())) + len(list(old_config['mcpServers']['oauth-server']['auth'].keys()))) - len(list(new_config['servers'][0].keys()))) / (len(list(old_config['mcpServers']['oauth-server'].keys())) + len(list(old_config['mcpServers']['oauth-server']['auth'].keys()))) * 100
